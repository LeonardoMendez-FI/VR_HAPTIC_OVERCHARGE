...
// Joystick izquierdo
const uint8_t PIN_JL_X  = 32;   // ADC1_CH4
const uint8_t PIN_JL_Y  = 33;   // ADC1_CH5
const uint8_t PIN_BTN_L = 25;   // botón del joystick izquierdo

// Joystick derecho
const uint8_t PIN_JR_X  = 34;   // ADC1_CH6
const uint8_t PIN_JR_Y  = 35;   // ADC1_CH7
const uint8_t PIN_BTN_R = 26;   // botón del joystick derecho

// ─────────────── UMBRALES PARA DIGITALIZAR ──────────────────
const uint16_t ADC_CENTER   = 2048;   // valor central del ADC (0-4095)
const uint16_t ADC_DEADZONE = 300;    // margen para reposo (±300)
...
void setup() {
  ...
  // Configurar botones con pull-up interno (LOW = pulsado)
  ...
  if (WiFi.softAP(AP_SSID, AP_PASSWORD)) {
    ...
    Serial.println("[ERROR] No se pudo crear el AP. Reiniciando...");
    ESP.restart();
  }
...
void loop() {
  if (millis() - ultimoEnvio >= SEND_MS) {
    ultimoEnvio = millis();
    enviarDatos();
    // Descomenta para depurar por Serial:
    // debugSerial();
  }
}

// Convierte la lectura analógica (0-4095) a -1, 0 o 1
